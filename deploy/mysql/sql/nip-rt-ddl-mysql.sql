...
create table if not exists nip_rt_article
(
...
CALL add_index_if_not_exists(
    ...
    'create index idx_article_catalog_id on nip_rt_article (catalog_id)'
);

CALL add_index_if_not_exists(
    'nip_rt_article',
    'idx_catalog_creator_order',
    'create index idx_catalog_creator_order on nip_rt_article (catalog_id, create_by, order_id)'
);

CALL add_index_if_not_exists(
    'nip_rt_article',
    'idx_content_text',
    'create fulltext index idx_content_text on nip_rt_article (content_text)'
);

create table if not exists nip_rt_article_history
(
...
CALL add_index_if_not_exists(
    'nip_rt_article_history',
    'idx_article_history_article_time',
    'create index idx_article_history_article_time on nip_rt_article_history (article_id, modified_time)'
);

create table if not exists nip_rt_catalog
(
...
CALL add_index_if_not_exists(
    ...
    'create index idx_catalog_father_id on nip_rt_catalog (father_id)'
);

create table if not exists nip_rt_section_summary
(
...
CALL add_index_if_not_exists(
    'nip_rt_section_summary',
    'idx_rt_summary_article_id',
    'create index idx_rt_summary_article_id on nip_rt_section_summary (article_id)'
);

-- auto-generated definition
create table if not exists nip_rt_share
(
...
CALL add_index_if_not_exists(
    'nip_rt_share',
    'idx_share_resource',
    'create index idx_share_resource on nip_rt_share (resource_type, resource_id)'
);

CALL add_index_if_not_exists(
    'nip_rt_share',
    'idx_share_target_role',
    'create index idx_share_target_role on nip_rt_share (target_role)'
);

CALL add_index_if_not_exists(
    'nip_rt_share',
    'idx_share_target_user',
    'create index idx_share_target_user on nip_rt_share (target_user)'
);

create table if not exists nip_rt_article_comment_thread
(
    ...
    constraint uk_nip_rt_comment_thread
        unique (article_id, thread_id)
)
    comment '文章批注线程';

CALL add_index_if_not_exists(
    'nip_rt_article_comment_thread',
    'idx_nip_rt_comment_thread_article',
    'create index idx_nip_rt_comment_thread_article on nip_rt_article_comment_thread (article_id)'
);

create table if not exists nip_rt_article_comment
(
    id          bigint auto_increment comment '主键'
        primary key,
    article_id  bigint                               not null comment '文章ID',
    ...
    create_time datetime   default CURRENT_TIMESTAMP not null comment '创建时间',
    update_time datetime   default CURRENT_TIMESTAMP not null comment '更新时间',
    constraint uk_nip_rt_comment
        unique (article_id, comment_id)
)
    comment '文章批注评论';

CALL add_index_if_not_exists(
    'nip_rt_article_comment',
    'idx_nip_rt_comment_thread',
    'create index idx_nip_rt_comment_thread on nip_rt_article_comment (article_id, thread_id)'
);
