...
create index idx_article_catalog_id
    on nip_rt_article (catalog_id);

create index idx_catalog_creator_order
    on nip_rt_article (catalog_id, create_by, order_id);

create fulltext index idx_content_text
    on nip_rt_article (content_text)
    comment '作为ES的降级或补充方案';

create table nip_rt_article_history
(
    id            bigint auto_increment comment 'ID'
        primary key,
    ...
    constraint uk_article_history_version
        unique (article_id, version_no)
)
    comment '文章历史版本表';

create index idx_article_history_article_time
    on nip_rt_article_history (article_id, modified_time);

create table nip_rt_catalog
(
...
create index idx_catalog_father_id
    on nip_rt_catalog (father_id);

create table nip_rt_section_summary
(
...
create index idx_rt_summary_article_id
    on nip_rt_section_summary (article_id);



-- auto-generated definition
create table nip_rt_share
(
...
create index idx_share_resource
    on nip_rt_share (resource_type, resource_id);

create index idx_share_target_role
    on nip_rt_share (target_role);

create index idx_share_target_user
    on nip_rt_share (target_user);




create table nip_rt_article_comment_thread
(
...
create index idx_nip_rt_comment_thread_article
    on nip_rt_article_comment_thread (article_id);



create table nip_rt_article_comment
(
    id          bigint auto_increment comment '主键'
        primary key,
    article_id  bigint                               not null comment '文章ID',
...
create index idx_nip_rt_comment_thread
    on nip_rt_article_comment (article_id, thread_id);
