...
public final class RichTextRedisScript {
     ...
     */
    public static final String ES_SYNC_PENDING_KEY = "nip:{article-es-sync}:pending";

    /**
     * 文章 ES 同步脏版本 Key 前缀，完整 Key 以文章 ID 结尾。
     * 每次文章发生持久化修改时版本号递增。
     */
    public static final String ES_SYNC_DIRTY_VERSION_KEY_PREFIX = "nip:{article-es-sync}:dirty-version:";

    /**
     * 文章 ES 同步分布式锁 Key 前缀，完整 Key 以文章 ID 结尾。
     */
    public static final String ES_SYNC_LOCK_KEY_PREFIX = "nip:article:es:sync:lock:";
